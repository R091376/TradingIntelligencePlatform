...
public final class StructureBarEvaluator {

    private StructureBarEvaluator() {
    }

    public static SimpleBarEvaluation evaluate(
            String symbolId,
            String timeframe,
            List<ActivePattern> openStructure,
        ...
        if (closedAscending == null || closedAscending.isEmpty()) {
            for (ActivePattern p : openStructure) {
        ...
        for (ActivePattern open : openStructure) {
            if (!open.patternType().isStructure() || open.isTerminal()) {
                continue;
            }
            List<PatternStageEvent> stageEvents = StructureLifecycle.onCandle(open, signal, config, now);
        ...
        StructureDetector.tryDetect(
            ...
            List<PatternStageEvent> sameBar = StructureLifecycle.onCandle(detected, signal, config, now);
